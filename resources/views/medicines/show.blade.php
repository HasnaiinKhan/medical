...
<div class="grid gap-8 lg:grid-cols-2">
    ...
    <div class="space-y-3" x-data="{ active: 0 }">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 via-blue-50 to-cyan-100 shadow-inner" style="height: 320px;">
            @php $allImages = $medicine->allImages(); @endphp
            <template x-for="(src, i) in {{ json_encode($allImages) }}" :key="i">
                <img :src="src" :alt="'{{ addslashes($medicine->name) }} image ' + (i+1)"
        ...
        <div class="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
            ...
            <span class="text-sm font-medium text-slate-800">In Stock — {{ $medicine->stock }} units available</span>
        </div>
    </div>
    ...
    <div class="flex flex-col">
        ...
        <div class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div class="flex items-baseline gap-3">
                ...
                @if ($medicine->mrp_paise > $medicine->price_paise)
                    ...
                    <span class="rounded-full bg-blue-100 px-2.5 py-0.5 text-sm font-bold text-slate-800">
            ...
            @if ($medicine->mrp_paise > $medicine->price_paise)
                <p class="mt-1 text-sm text-slate-700 font-medium">
                    You save ₹{{ number_format($medicine->mrpRupees() - $medicine->priceRupees(), 2) }} on this item
                </p>
            @endif
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-3">
                ...
                <div class="flex flex-wrap gap-1.5">
                    ...
                    <span class="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">
                            <div class=lock style="width:20px;">
                            <img src="{{ asset('images/dollars.png') }}"
                            alt="lock"> 
                            </div>
                    COD</span>
                </div>
        ...
        <form method="post" action="{{ route('cart.add') }}" class="js-add-to-cart-form w-full mt-6 {{ $cartQuantity > 0 ? 'hidden' : '' }}">
            ...
            <button type="submit"
                    class="btn-primary w-full rounded-2xl py-4 text-sm font-black text-white shadow-sm">
                🛒 Add to Cart
            </button>
        </form>
    ...
    <section class="mt-12 border-t border-slate-200 pt-10">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            @foreach ($related as $r)
                <article data-product-url="{{ route('medicines.show', $r) }}" data-product-id="{{ $r->id }}" class="medicine-card cursor-pointer flex flex-col rounded-2xl bg-white shadow-sm overflow-hidden transition-shadow hover:shadow-md">
                    ...
                    <div class="flex flex-1 flex-col p-3">
                        ...
                        <div class="mt-auto pt-3">
                            <form method="post" action="{{ route('cart.add') }}" class="js-add-to-cart-form w-full {{ isset($cartItems[$r->id]) && $cartItems[$r->id] > 0 ? 'hidden' : '' }}">
                                @csrf
                                <input type="hidden" name="medicine_id" value="{{ $r->id }}">
                                <button type="submit" class="btn-primary w-full rounded-2xl py-3 text-sm font-black text-white shadow-sm">Add to Cart</button>
                            </form>

                            <form method="post" action="{{ route('cart.update', $r) }}"
                                  class="js-cart-update-form flex w-full items-center justify-between rounded-2xl border border-slate-200 overflow-hidden mt-3 {{ isset($cartItems[$r->id]) && $cartItems[$r->id] > 0 ? '' : 'hidden' }}"
                                  data-cart-medicine-id="{{ $r->id }}">
                                @csrf
                                @method('PATCH')
                                <button type="button"
                                        class="js-card-qty-minus w-14 bg-slate-50 text-slate-700 hover:bg-slate-100 transition-colors font-bold text-lg leading-none"
